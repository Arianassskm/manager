...
require './need.php';
$QQ = @$_REQUEST['QQ'];//登录者的QQ号
$Skey = @$_REQUEST['Skey'];//登录者的skey
$Pskey = @$_REQUEST['Pskey'];//登录者的pskey(vip.qq.com)
$uin = @$_REQUEST['uin'];//需要查询的QQ号
...
Switch($Type){
    case 'text':
    need::send('账号：'.$uin."".$hh."昵称：{$Name}".$hh."等级：{$Level}".$hh."等级换算入网时长：{$Day}天".$hh."距离下一级还差{$Next}天(未加速)",'text');//今日成长：{$TheDays}天".$hh."今日非隐身在线时长{$Time}小时
    break;
    default:
    $arr = [];
    $arr['uin'] = $uin;
